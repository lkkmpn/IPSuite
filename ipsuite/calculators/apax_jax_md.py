...
class ApaxJaxMD(base.ProcessSingleAtom):
...
class MappedApaxJaxMD(base.ProcessAtoms):
    """Class to run a more performant JaxMD simulation with a apax Model.
    ...
    def run(self):
        ...
        self._handle_parameter_file()
        atoms_lst = self.get_atoms()

        for atoms in atoms_lst:
            ase.io.write(self.init_struc_dir.as_posix(), atoms, append=False)
